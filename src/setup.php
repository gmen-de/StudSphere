...
function installDatabase(): void
{
    ...
    $queries = [
        ...
        // One row per physical minifig — no quantity column, mirrors
        // owned_sets (one row per physical set copy) rather than
        // storage_items (one aggregated row per part/color/condition). Each
        // row's own completeness lives in minifig_storage_item_parts.
        ...
        // minifig_id survives the instance's own deletion deliberately (ON
        // DELETE SET NULL, not CASCADE) — mirrors owned_set_sales, a sale
        // record is a permanent log entry, not tied to the storage row's
        // lifecycle (sellOwnedMinifigInstance() always deletes the instance
        // right after inserting here).
        ...
        // Inventur (re-count) for owned sets and flagged storage locations —
        // see src/stocktakes.php. Its own tables rather than reusing
        // pick_lists/pick_list_items: a stocktake never physically relocates
        // stock, it only corrects the quantity already sitting at the same
        // location.
