...
/**
 * Deletes a pick list the user changed their mind about — only when its own
 * location genuinely holds nothing right now (same live check
 * closePickListIfEmpty() uses, not pick_list_items.picked_quantity, which
 * doesn't reset after a put-away: a list that was picked into and then
 * fully put away away again is legitimately empty and deletable, even
 * though its items still show picked_quantity > 0 historically). While
 * anything IS physically sitting at this list's storage_locations row,
 * deleting it out from under that stock would either orphan it (the FK from
 * storage_items to storage_locations is ON DELETE RESTRICT, so the location
 * couldn't actually be removed anyway) or silently destroy it, neither of
 * which is an acceptable "delete a list" outcome — the correct path for a
 * partially-picked list the user wants to abandon is the put-away assistant
 * (getPutAwaySuggestions()/putAwayItem()), not deletion.
 *
...
function deletePickList(PDO $pdo, int $pickListId): void
{
    ...
    $hasStock = !empty(getLocationStock((int) $pickList['location_id']));
    $hasMinifigs = (int) $pdo->query(
        'SELECT COUNT(*) FROM minifig_storage_items WHERE location_id = ' . (int) $pickList['location_id']
    )->fetchColumn() > 0;
    if ($hasStock || $hasMinifigs) {
